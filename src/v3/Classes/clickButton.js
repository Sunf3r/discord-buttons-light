const { sendAPICallback } = require('discord-buttons/src/v3/Classes/APIMessage');
const WebhookClient = require('discord-buttons/src/v3/Classes/WebhookClient');
...
class MessageComponent {
    ...
    constructor(client, data) {
        ...
        this.clicker = {
            ...
            fetch: async () => {
                this.clicker.user = await this.client.users.forge(data.guild_id ? data.member.user.id : data.user.id);
                if (this.guild) {
                    this.clicker.member = await this.guild.members.forge(data.member.user.id);
                }
                return true;
            }
        };
